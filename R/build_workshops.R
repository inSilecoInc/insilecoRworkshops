...
build_workshops <- function(path = ".", download = FALSE, id = NULL,
  ...
  cli::cli_alert_success(
    paste0(
      paste(gsub(".*/", "", Rmdfiles), collapse = ", "),
      " successfully rendered."
    )
  )

  if (pdf) {
    cli::cli_alert_info("converting html files to pdf")
    html <- gsub("\\.[Rr]md$", "\\.html", Rmdfiles)
    lapply(html, chrome_print)
    cli::cli_alert_success(paste0(
      "PDF file", ifelse(length(html) > 1, "s", ""),
      " successfully created."
    ))
  }
  if (script) {
    cli::cli_alert_info("extracting R code from the R Markdown files")
    extract_Rcode_workshops(path, verbose = FALSE)
    cli::cli_alert_success(paste0(
      "R script", ifelse(length(Rmdfiles) > 1, "s", ""),
      " successfully created."
    ))
  }
